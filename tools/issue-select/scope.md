...
I'm a forward deployed engineer who builds and deploys LLM and RAG systems, so I'm most at home in Python (backend services, pytest) and in retrieval, chunking, and output-parsing code. I've also written JavaScript/TypeScript and done docs and DevOps work (READMEs, CI, deployment). For a first contribution I want a Python bug in the RAG or ingestion pipeline that I can reproduce with a failing test and fix in a few hours. I'd rather avoid frontend-only work and large tier-3 features.
